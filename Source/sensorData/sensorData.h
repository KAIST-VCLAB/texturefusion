...
#define M_SENSOR_DATA_VERSION 4
		...
		private:
			...
			static void decompFunc(RGBDFrameCacheRead* cache) {
				while (1) {
					int turn = cache->m_nextFromSensorData / cache->m_sensorData->m_frames.size();
					int th = cache->m_nextFromSensorData  % cache->m_sensorData->m_frames.size();

					//20190918 
					if (turn % 2) {
						th = cache->m_sensorData->m_frames.size() - th - 1;
						cache->check = true;
					}
//					printf("turn th: %d %d %d\n", turn, th, cache->m_sensorData->m_frames.size());

					if (cache->m_bTerminateThread) break;

					//if (cache->m_nextFromSensorData >= cache->m_sensorData->m_frames.size()) {
					//	
