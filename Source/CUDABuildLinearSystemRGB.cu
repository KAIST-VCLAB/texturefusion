...
__shared__ float bucket3[ARRAY_SIZE*BLOCK_SIZE];
...
__device__ inline void CopyToResultScanElement(uint GID, float* output)
{
	...
	for (uint i = 0; i<ARRAY_SIZE; i++)
	{
		output[ARRAY_SIZE*GID+i] = bucket3[0+i];
	}
}
...
__device__ inline void SetZeroScanElement(uint GTid)
{
	...
	for(uint i = 0; i<ARRAY_SIZE; i++)
	{
		bucket3[GTid*ARRAY_SIZE+i] = 0.0f;
	}
}
...
{
	addToLocalScanElement(GTid + 32, GTid, bucket3);
	addToLocalScanElement(GTid + 16, GTid, bucket3);
	addToLocalScanElement(GTid + 8 , GTid, bucket3);
	addToLocalScanElement(GTid + 4 , GTid, bucket3);
	addToLocalScanElement(GTid + 2 , GTid, bucket3);
	addToLocalScanElement(GTid + 1 , GTid, bucket3);
}
...
__global__ void scanNormalEquationsRGBDevice(unsigned int imageWidth, unsigned int imageHeight, float* output, CameraTrackingInput cameraTrackingInput, float3x3 intrinsics, CameraTrackingParameters cameraTrackingIParameters, float3 anglesOld, float3 translationOld, unsigned int localWindowSize)
{
			...
			if (!pInput.checkMINF() && !nInput.checkMINF() && !iInput.checkMINF())
			{
				...
				if (pProjTrans(2) > 0.0f)
				{
					...
					if (!iTarget.checkMINF() && !iTargetDerivUIntensity.checkMINF() && !iTargetDerivVIntensity.checkMINF())
					{
						...
						addToLocalSystem(jacobianBlockRowIntensity, diffIntensity, cameraTrackingIParameters.weightColor*weightColor, threadIdx.x, bucket3);
	...
	{
		if (threadIdx.x < stride) addToLocalScanElement(threadIdx.x+stride/2, threadIdx.x, bucket3);
...
extern "C" void computeNormalEquationsRGB(unsigned int imageWidth, unsigned int imageHeight, float* output, CameraTrackingInput cameraTrackingInput, float* intrinsics, CameraTrackingParameters cameraTrackingParameters, float3 anglesOld, float3 translationOld, unsigned int localWindowSize, unsigned int blockSizeInt)
{
	...
	scanNormalEquationsRGBDevice<<<gridSize, blockSize>>>(imageWidth, imageHeight, output, cameraTrackingInput, float3x3(intrinsics), cameraTrackingParameters, anglesOld, translationOld, localWindowSize);
	#ifdef _DEBUG
