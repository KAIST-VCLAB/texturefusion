...
#pragma unroll 1
	...
	//used
	__device__
		void traverseCoarseGridSimpleSamplePrevTexture(const HashData& hash, const TexPoolData& texPoolData, const DepthCameraData& cameraData, const float3& worldPos, const float3& worldDir, const int& texel_addr, float minInterval, float maxInterval) const
	{
		const RayCastParams& rayCastParams = c_rayCastParams;
				...
				if (lastSample.weight > 0 && lastSample.sdf > 0.0f && dist < 0.0f) // current sample is always valid here 
				{
					...
					if (b && abs(lastSample.sdf - dist) <rayCastParams.m_thresDist) //there is bisection and the step is under threshold.
					{
						...
						if (abs(dist) < rayCastParams.m_thresDist)
						{
							...
							texPoolData.d_texPatches[texel_addr].color_dummy = texelPrev.color;
