...
struct Voxel {
	...
	__device__ void operator=(const struct Voxel& v) {
		...
		((long long*)this)[1] = ((const long long*)&v)[1];
		((int*)this)[4] = ((const int*)&v)[4];
		((uchar*)this)[20] = ((const uchar*)&v)[20];
		((uchar*)this)[21] = ((const uchar*)&v)[21];
	}
...
struct HashData {
	...
	HashData() {
		...
		d_voxelZeroCross = NULL;
		d_voxelZeroCrossCounter = NULL;
	...
	void allocate(const HashParams& params, bool dataOnGPU = true) {
		...
		if (m_bIsOnGPU) {
			...
			//ZeroCross index
			cutilSafeCall(cudaMalloc(&d_voxelZeroCross, sizeof(int3)* params.m_hashNumBuckets * 2));
			cutilSafeCall(cudaMalloc(&d_voxelZeroCrossCounter, sizeof(int)));
...
//#define __CUDACC__
...
#endif
		// set values of a voxel
    	out.color.x = (uchar)(res.x+0.5f);	out.color.y = (uchar)(res.y+0.5f); out.color.z = (uchar)(res.z+0.5f);		
		out.prev_sdf = v0.prev_sdf;
		out.prev_texind = v0.prev_texind;
		out.prev_weight = v0.prev_weight;
		out.texind = v0.texind;
		out.isUpdated = 1;
	...
	int3*		d_voxelZeroCross;			//same as before except that only valid pointers are there
	int*		d_voxelZeroCrossCounter;	//atomic counter to add compactified entries 
